  ...
  <div class="pagetitle">
    ...
    <nav>
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Edit: {{ Str::limit($tourPackage->title, 35) }}</li>
      </ol>
    </nav>
  </div>
  ...
  <section class="section">
    <div class="row">
      <div class="col-lg-12">
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">Edit Package: {{ $tourPackage->title }}</h5>
            ...
            <form id="tour-edit-form" method="POST" action="{{ route('admin.tour-packages.update', $tourPackage->id) }}" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <input type="text" name="title" class="form-control"
                         value="{{ old('title', $tourPackage->title) }}" required>
                </div>
              </div>
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <input type="text" name="slug" class="form-control"
                         value="{{ old('slug', $tourPackage->slug) }}" required>
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  {{--
                      ...
                      Existing safari car images uploaded the old way were migrated
                      into this same system via the one-time `media:migrate-galleries`
                      command.
                  --}}
                  <x-media-picker
              ...
              @endphp
              @include('admin.tour-packages.partials.pricing.selector', [
              ...
              <div class="row mb-4 mt-5">
                ...
                <div class="col-sm-10">
                  <div id="itinerary-repeater">
                    @if(old('itinerary_days', $tourPackage->itinerary ?? []))
                      @foreach(old('itinerary_days', $tourPackage->itinerary ?? []) as $index => $day)
                        <div class="itinerary-day card mb-3 shadow-sm" data-day-index="{{ $index }}">
                          <div class="card-header d-flex justify-content-between align-items-center bg-success">
                            <h6 class="mb-0 text-white">Day {{ $loop->iteration }}</h6>
                            ...
                            <div class="row g-3">
                              <div class="col-md-12">
                                ...
                                <input type="text" name="itinerary_days[{{ $index }}][title]"
                                       class="form-control"
                                       value="{{ old("itinerary_days.$index.title", $day['title'] ?? '') }}">
                              </div>
                              ...
                              <div class="col-md-12">
                                ...
                                <input type="hidden" name="itinerary_days[{{ $index }}][location_name]" class="loc-field-location_name" value="{{ old("itinerary_days.$index.location_name", $day['location_name'] ?? '') }}">
                                <input type="hidden" name="itinerary_days[{{ $index }}][lat]" class="loc-field-lat" value="{{ old("itinerary_days.$index.lat", $day['lat'] ?? '') }}">
                                <input type="hidden" name="itinerary_days[{{ $index }}][lng]" class="loc-field-lng" value="{{ old("itinerary_days.$index.lng", $day['lng'] ?? '') }}">
                              </div>

                              {{--
                                  Picker-only, per decision: direct upload and the old
                                  per-image remove/detach UI removed entirely. The
                                  field name (existing_image_ids[]) is unchanged, so
                                  TourPackageController's existing JSON-construction
                                  logic for itinerary[].image_ids keeps working without
                                  modification — only how the value gets populated has
                                  changed, from file upload to picker selection.
                              --}}
                                  ...
                                  <div class="row g-2 align-items-end mb-3 accommodation-tier-row">
                                    ...
                                    <div class="col-md-9">
                                      ...
                                      {{--
                                          ...
                                          (existing_accommodation_image_{tier}) unchanged,
                                          so TourPackageController's existing tier-image
                                          resolution logic keeps working without
                                          modification — only how the value gets
                                          populated has changed.
                                      --}}
                                      @php $accExistingImageId = old("itinerary_days.$index.existing_accommodation_image_$tierKey", $existingImageId); @endphp
                                      <x-media-picker
                              ...
                              <div class="col-md-6">
                                ...
                                <input type="text" name="itinerary_days[{{ $index }}][meals]"
                                       class="form-control"
                                       value="{{ old("itinerary_days.$index.meals", $day['meals'] ?? '') }}">
                              </div>
                            </div>
                          </div>
                        </div>
                  ...
                  {{--
                      Hidden template pickers for brand-new days added client-side via
                      the "Add New Day" button below. <x-media-picker> is a server-
                      rendered Blade component, so a genuinely new day index (one that
                      didn't exist when this page loaded) can't get a freshly-rendered
                      picker from the server without a round trip. Instead, one
                      fully-rendered picker per slot (day images + 3 accommodation
                      tiers) is rendered here once, with a placeholder index
                      ("__INDEX__") in every name/id attribute, and the "Add New Day"
                      script below clones this whole hidden block and string-replaces
                      "__INDEX__" with the real new day number — which works because
                      MediaPicker::$pickerId is deterministically derived from $name
                      (see App\View\Components\MediaPicker), so renaming the input's
                      name and renaming its modal/JS hookup id are the same find-and-
                      replace operation.
                  --}}
                  ...
                  <div class="row">
                    <div class="col-md-6">
                      ...
                      <div id="inclusions-repeater">
                        @if(old('inclusions_items', $tourPackage->inclusions ?? []))
                          @foreach(old('inclusions_items', $tourPackage->inclusions ?? []) as $index => $item)
                    ...
                    <div class="col-md-6">
                      ...
                      <div id="exclusions-repeater">
                        @if(old('exclusions_items', $tourPackage->exclusions ?? []))
                          @foreach(old('exclusions_items', $tourPackage->exclusions ?? []) as $index => $item)
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <textarea name="highlights_text" class="form-control" rows="6" placeholder="One highlight per line">{{ old('highlights_text', $tourPackage->highlights ? implode("\n", $tourPackage->highlights) : '') }}</textarea>
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <textarea name="meta_description" class="form-control" rows="3">{{ old('meta_description', $tourPackage->meta_description) }}</textarea>
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <input type="text" name="meta_keywords" class="form-control"
                         value="{{ old('meta_keywords', $tourPackage->meta_keywords) }}">
                ...
                <div class="col-sm-10">
                  <div class="form-check form-switch mt-2">
                    <input type="checkbox" class="form-check-input" id="no_robots_edit" name="no_robots" value="1"
                           {{ old('no_robots', $tourPackage->no_robots) ? 'checked' : '' }}>
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-5">
                  <select name="status" class="form-select">
                    ...
                    <option value="archived"  {{ old('status', $tourPackage->status) == 'archived'  ? 'selected' : '' }}>Archived</option>
                  </select>
                </div>
                <div class="col-sm-5">
                  <div class="form-check mt-2">
...
{{-- ▼ NEW: Accommodation image remove handler --}}
<script>
  document.addEventListener('click', function (e) {
...
{{-- ▼ NEW: Extra section image remove handler --}}
<script>
  document.addEventListener('click', function (e) {
