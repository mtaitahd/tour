<script>
window.__pricing = {
    levelCatalog: @json($levelCatalog),
    predefinedCosts: @json($predefinedCosts),
    basisLabels: @json($basisLabels),
    singleDayLevel: @json($singleDayLevel),
    seasons: ['HIGH', 'LOW_WET'],
    manualRows: @json($manualRows ?? []),
    existingPayload: @json($existingCalculatorPayload),
    source: @json($currentSource),
    duration: @json($currentDuration),
    category: @json($currentCategory),
    tourType: @json($currentTourType),
    previewUrl: @json(route('admin.tour-price-calculator.preview')),
    baseCurrency: @json($currentManualCurrency ?? 'USD'),
    items: [],
